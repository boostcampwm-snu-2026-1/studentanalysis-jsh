...
- [x] feature/9: 학생 상세 — 기본정보 탭
  - [x] 탭 레이아웃 컴포넌트 (기본정보 / 생기부 분석 / 상담 기록 / 대학 탐색)
  - [x] 인적사항 카드 (이름·학년·반·번호·목표대학·목표계열) + 수정 폼
  - [x] 내신 입력 팝업 — 학기 선택, 과목명·등급 행 추가/삭제
  - [x] 내신 학기별 평균·전체 평균 자동 계산 표시
  - [x] 모의고사 입력 팝업 — 연월 입력, 영역별 등급·백분위 입력
  - [x] 내신 성적 테이블 렌더링
  - [x] 모의고사 히스토리 테이블 렌더링

- [x] feature/10: 학생 상세 — 생기부 분석 탭
  - [x] 생기부 원문 텍스트에어리어 + 분석 시작 버튼
  - [x] 5단계 진행 표시 로딩 UI (단계별 상태 표시)
  - [x] 단계별 수동 재시도 버튼 (실패 시 노출)
  - [x] 1단계 역량 프로필 — 레이더/바 차트 (4개 점수)
  - [x] 2단계 종합 진단 — 강점·보완점·추천 전공 카드
  - [x] 3·4단계 활동 추천 — 유형별 카드 (안정·심화·차별화·실천)
